...
def expand_bbox(left, right, top, bottom, img_width, img_height):   #xmin,xmax坐标，ymin,ymax坐标，图片宽高
    width = right-left
    height = bottom-top
    ratio = 0.15
    ...
    return [int(new_left), int(new_top), int(new_right), int(new_bottom)]   #框住人所在的区域，即矩形框的左上角和右下角
...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        cv2.imwrite(f"./cv2img/{0}.jpg",image)
        print( image.shape)
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        #         txt.write(str(image[0][j, i]))
        #
        #         txt.write("\r")
        #
        # txt.close()
        #print("________",image)
...
class PoseDataset(Dataset):
    ...
    def __init__(self, csv_file, transform):
        l = []
        self.root = os.path.dirname(csv_file)   #去掉文件名，返回目录
        with open(csv_file) as f:
            self.f_csv = csv.reader(f,delimiter='\t')
            for r in self.f_csv:
                l.append(r)
        self.f_csv = l
    ...
    def __getitem__(self, idx):
        ...
        image = io.imread(img_path)     #直接转成numpy
        ...
        pose = (pose-np.array([box[0],box[1]])).flatten()   #flatten()按行降维，即二维数组变成一维
